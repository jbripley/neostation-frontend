  ...
  static Future<List<DatabaseGameModel>> getGamesBySystem(
    ...
    final results = await db.rawQuery(
      ...
      SELECT
        ...
        COALESCE(usm.real_name, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.title_name END, ur.filename) as game_display_name,
        usm.real_name as ss_real_name,
        COALESCE(usm.description_en, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.description END) as description,
        usm.description_en, usm.description_es, usm.description_fr, usm.description_de, usm.description_it, usm.description_pt,
        usm.rating,
        COALESCE(usm.release_date, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.year END) as year,
        COALESCE(usm.developer, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.developer END) as developer,
        COALESCE(usm.publisher, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.publisher END) as publisher,
        COALESCE(usm.genre, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.genre END) as genre,
        COALESCE(usm.players, CASE WHEN s.folder_name IN ('android', 'androidgames') THEN ur.players END) as players,
